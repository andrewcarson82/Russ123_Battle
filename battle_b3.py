...
def draw_panel():
	...
	for i, unit in enumerate(player_team):
		...
		draw_text(f'{unit.name}: HP {unit.hp}/{unit.max_hp} ATK {unit.atk} DEF {unit.def_stat}', 
				 pygame.font.SysFont('Arial', 16), red, 20, y_pos)
	
	# Draw AI team stats
	draw_text("AI TEAM", font, red, 450, screen_height - bottom_panel + 10)
	for i, unit in enumerate(ai_team):
		y_pos = screen_height - bottom_panel + 35 + (i * 25)
		draw_text(f'{unit.name}: HP {unit.hp}/{unit.max_hp}', 
				 pygame.font.SysFont('Arial', 16), red, 450, y_pos)
...
# Modified main game initialization
def main():
...
while run:
	...
	# Simple click-to-attack for testing
	for unit in ai_team:
		if unit.rect.collidepoint(pos) and unit.alive:
			...
			if clicked:
				# Find first alive player unit to attack
				attacker = None
				for player_unit in player_team:
					if player_unit.alive:
						attacker = player_unit
						break
				
				if attacker:
					damage = attacker.attack(unit)
					print(f"{attacker.name} attacks {unit.name} for {damage} damage!")
